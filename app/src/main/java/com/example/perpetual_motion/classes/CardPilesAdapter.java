package com.example.perpetual_motion.classes;
...
// The message to show on the bottom of the outer card (e.g "Cards in Stack: ")
    ...
    private void updateOuterCard (CardPileTopViewHolder holder, int
            position)
    {
        holder.tv_pile_card_cards_in_stack.setText (
                mMSG_CARDS_IN_STACK.concat (Integer.toString
                        (mNUMBER_OF_CARDS_IN_PILE[position])));
    }
    ...
    {
        return mPILE_TOPS[position] == null ? null :
                mPILE_TOPS[position].copy ();
    }
    ...
    {
        System.arraycopy (newChecksSet, 0, mCHECKED_PILES, 0,
                mCHECKED_PILES.length);
    }
    ...
    private void animateCard (View view)
    {
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            int endRadius = Math.max (view.getWidth (), view.getHeight
                    ());
            Animator circularRevealAnimationOfCard =
     ...
     */
    public void setOnItemClickListener (AdapterOnItemClickListener
    ...
    private void populateAndShowInnerCard (CardPileTopViewHolder holder, int position)
    {
        ...
        holder.cb_pile_card_checkbox.setChecked
                (mCHECKED_PILES[position]);
        holder.cv_pile_inner_Card.setVisibility (View.VISIBLE);
    }
    private void setTextOfEachTextView (CardPileTopViewHolder holder, Card
            currentCard)
    {
        String rankValue = String.format(Locale.getDefault (), "%d",
                currentCard.getRank ().getValue ());
        holder.tv_pile_card_rank_top.setText (rankValue);
        holder.tv_pile_card_suit_center.setText (Character.toString
                (currentCard.getSuit ().getCharacter ()));
        holder.tv_pile_card_name_bottom.setText (currentCard.getRank
                ().toString ());
    }

    private void setColorOfAllTextViews (CardPileTopViewHolder holder, Card
            currentCard)
    {
